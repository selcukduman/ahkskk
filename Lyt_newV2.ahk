...
Loop Lyt.GetList().MaxIndex()
	str .= A_Index ": " Lyt.GetList()[A_Index].LocName " - " Lyt.GetList()[A_Index].LayoutName	. "`n" Format("{:#010x}", Lyt.GetList()[A_Index].hkl) "`n"

MsgBox(str "`n" defaulSystemLayout, "Your system loaded layout list", "")
...
F12::MsgBox("HKL: " Format("{:#010x}", Lyt.GetInputHKL()) "`nâ„–: " Lyt.GetNum() " in system loaded list" . "`nLayoutName: " Lyt.GetLayoutName() "`nLocale: " Lyt.GetLocaleName() . " (" Lyt.GetLocaleName(,, true) ")`n" Lyt.GetList()[3].KLID)
...
Class Lyt {
		...
		hWnd := (win = "")
						...
						: ( win + 0
								...
								: win = "global"
									...
									: WinExist(win) )
		if (hWnd = 0)
			Return "Window not found" ; WinExist() return 0
	...
	Change(HKL, INPUTLANGCHANGE, hWnd) {
		PostMessage(this.WM_INPUTLANGCHANGEREQUEST, HKL ? "" : INPUTLANGCHANGE, HKL ? HKL : "", , "ahk_id" ((hWndOwn := DllCall("GetWindow", "Ptr", hWnd, "UInt", GW_OWNER:=4, "Ptr")) ? hWndOwn : hWnd))
	}
	...
	GetList() { ; List of system loaded layouts
		...
		Else {
			...
			VarSetStrCapacity(&List, A_PtrSize*Size) ; V1toV2: if 'List' is NOT a UTF-16 string, use 'List := Buffer(A_PtrSize*Size)'
			Size := DllCall("GetKeyboardLayoutList", "Int", Size, "Str", aList.Length)
			Loop Size {
				aLayouts[A_Index] := {}
				aLayouts[A_Index].hkl := NumGet(aList.Length, A_PtrSize*(A_Index - 1), "UPtr")
	...
	GetLocaleName(win := "", HKL := false, FullName := false) { ; e.g. "EN"
		...
		localeSig := Buffer(Size, 0) ; V1toV2: if 'localeSig' is a UTF-16 string, use 'VarSetStrCapacity(&localeSig, Size)'
	...
	GetKLIDfromHKL(HKL) {
		VarSetStrCapacity(&KLID, 8 * (1 ? 2 : 1)) ; V1toV2: if 'KLID' is NOT a UTF-16 string, use 'KLID := Buffer(8 * (A_IsUnicode ? 2 : 1))'
	...
	GetInputHKL(win := "") {
		...
		if (class == "ConsoleWindowClass") {
				...
				VarSetStrCapacity(&KLID, 16) ; V1toV2: if 'KLID' is NOT a UTF-16 string, use 'KLID := Buffer(16)'
